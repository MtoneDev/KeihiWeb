<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master" CodeBehind="Form_NYKINDN_New.aspx.vb" Inherits="KeihiWeb.Form_NYKINDN_New" %>
        ...
        for (var i = 1, l = tr.length ; i < l; i++) {
            ...
            var wkelment_div = document.createElement("div_bumon_h" + i);
               ...
               // var ojb = document.getElementById("hidden5");
                ...
                //支払
                var namewk = 'h_before_shiharai' + i;
                var wkelment_div = document.createElement("div_shiharai_h" + i);
                wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
                //var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
...
//エンターキーの処理
    $('input[type = text]').bind("keydown", function (e) {
        ...
        if (e.which == 13 || e.which == 9) {
            ...
            if ($('input[type = text]').index(this) === 8) {
                //ユーザーコードエンターはボタンにフォーカスさせて名称表示
                ...
                if (nextIndex < n) {
                    if ($('input[type = text]')[nextIndex].readOnly == true) {
                        $('#dialog_syukin').siblings('.ui-dialog-buttonpane').find('button:eq(0)').focus();
                    } else {

                        if ($('input[type = text]')[nextIndex].disabled == true) {

                            $('input[type = text]')[nextIndex + 1].focus();
                        } else {
    ...
    for (var i = 1, l = tr.length ; i < l; i++) {
        ...
        cells.eq(11).addClass('grd1_keihi');
        cells.eq(12).addClass('grd1_zei');
        cells.eq(13).addClass('grd1_kin');

    }

    //タイトルの変更
     if ($('#<%=syubetu.ClientID%>').val() === '新規') {
            $('#title').text(' 入金伝票入力（新規）');
            $('#<%=kounyu_date.ClientID%>').datepicker('setDate', new Date());
            $('#<%=button_delete.ClientID%>').prop('disabled', true);
   
     } else {
            $('#title').text(' 入金伝票入力（訂正）');
    ...
    var url_wk = 'Form_SYKINDN_New.aspx/GetLastCloseDate';
    var JSONdata = {
        code: jibumoncd,
    };


    $('#d_user_cd').prop('disabled', false); 
    $('#<%=kounyu_date.ClientID%>').prop('disabled', false);
    $('#user_serach').prop('disabled', false);
    //CLOSEDATEの取得 
    $.ajax({
        type: "POST",
        url: url_wk,
        ...
        success: function (result) {
            ...
            if (closedate >= inputdate_wk) {
                ...
                $('#d_user_cd').prop('disabled', true); 
                $('#<%=kounyu_date.ClientID%>').prop('disabled', true);
                $('#user_serach').prop('disabled', true);
    ...
    $('#<%=kounyu_date.ClientID%>').on({
        change: function () {
           ...
           if ($('#<%=kounyu_date.ClientID%>').val().length > 10) {
            ...
            var jibumoncd = document.getElementById("auth_bumon_cd").value;
            var url_wk = 'Form_NYKINDN_New.aspx/GetLastCloseDate';
            var JSONdata = {
                code: jibumoncd,
            };
            //CLOSEDATEの取得 
    ...
    $('#d_user_cd').on({
        change: function () {
            ...
            if (user_cd.length > 0) {
                ...
                $('#d_user_name').val('ユーザーIDの値が不正です');
                $('#d_user_name').css('color', 'red');
    ...
    $('#button_close').click(function (e) {
        ...
        var chk = '';
        var wkuser = $('#<%=auth_user_id.ClientID%>').val();
        var userwk = $('#d_user_cd').val();
        ...
        for (var i = 1, l = tr.length ; i < l; i++) {
            ...
            //支払
            var namewk = 'h_before_shiharai' + i;
            if (document.getElementById(namewk).value === shiharai) {
            } else {
                chk = '1';
            }
...
<asp:label runat="server" class="zandaka_lbl">残高</asp:label><asp:TextBox cssclass="input-sm zandaka" ID="zandaka" tabindex="-1" runat="server" readonly="true" ></asp:TextBox>
   ...
   <table class="header_group1">
    <tr>
     <td>購入日<asp:TextBox cssclass="div_konyudate div_kounyu_date　datepicker input-sm" id="kounyu_date" runat="server" style="margin-left:5px" ></asp:TextBox></td>
     <td></td> 
    </tr>
      <tr>
         ...
         <td><input type="text" name="d_user_name" id="d_user_name"  tabindex="-1" class="username input-sm" style="background-color:whitesmoke;border-style: none;width:200px" readonly="readonly"/></td>
       </tr>
</table>
        </asp:Panel>
    </div>
...
<!--新規登録画面の開始-->
<div id="dialog_syukin" class="dialog_syukin" title="入金伝票">
 <input type="hidden" id="d_gyo" />
<div > 
<label id="lable_bumon">部　門</label><input type="text" id="d_bumon_id" class="input-sm" style="font-size:small;width:100px;margin-left:5px;margin-top:10px"/><input id="bumon_serach" type="button" value="..." class="btn btn-default">
 <label id="d_bumon_name" ></label>
</div>
<div>
<label id="lable_nyu">入金先</label><input type="text" class="input-sm" id="d_shiharai" style="font-size:small;margin-left:5px;margin-top:10px;width:500px" />
</div>
<div>
<label id="lable_kamoku">科　目</label><input type="text" id="d_kan_code" class="input-sm" style="font-size:small;width:100px;margin-left:5px;margin-top:10px"/><input id="kan_serach" type="button" value="..." class="btn btn-default">
<label id="d_kan_name"></label>
</div>
<div>
<label id="lable_uchi">内　訳</label><input type="text" id="d_uti_code" class="input-sm" style="font-size:small;width:100px;margin-left:5px;margin-top:10px"/><input id="uti_serach" type="button" value="..." class="btn btn-default">
<label id="d_uti_name"></label>
</div>
<div>
<label id="lable_tekiyo">摘　要</label><input type="text" id="d_tekiyo" class="input-sm" style="font-size:small;margin-left:5px;margin-top:10px;width:500px" />
</div>
<div >
...
<div>
<label id="lable_kin">金　額</label><input type="text" id="d_kingaku" class="input-sm" style="font-size:small;margin-left:5px;margin-top:10px;text-align:left;width:150px" />
</div>
