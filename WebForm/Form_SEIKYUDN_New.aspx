<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master" CodeBehind="Form_SEIKYUDN_New.aspx.vb" Inherits="KeihiWeb.Form_SEIKYUDN_New" %>
...
<script type="text/javascript" src="../Scripts/WebForms/STSCommon/keihi_zeiritu2.js" ></script>
...
$(function() {
    ...
    function value_set() {
        ...
        for (var i = 1, l = tr.length ; i < l; i++) {
            ...
            var kamokucd = cells.eq(3).text();
            var kamokunm = cells.eq(4).text();
            var uchiwakecd = cells.eq(5).text();
            var uchiwakenm = cells.eq(6).text();
            var tekiyo = cells.eq(7).text();
            var zkubun = cells.eq(8).text();
            var zname = cells.eq(9).text();
            var keihi = cells.eq(10).text();
            var zei = cells.eq(11).text();
            var kin = cells.eq(12).text();

            if ((keihi.trim().length > 0) && (zei.trim().length > 0) && (kin.trim().length > 0)) {
                if (parseInt(keihi.replace(/,/g, '')) + parseInt(zei.replace(/,/g, '')) == parseInt(kin.replace(/,/g, ''))) {
    ...
    //初期値項目保存関数（グリッドの値をhiddn項目へセットする関数）
    function value_set2() {
        var dom_obj = document.getElementById("hidden15");
        dom_obj.textContent = null;

        $('#<%=keihi_goukei.ClientID%>').val($('#<%=keihi_goukeiran.ClientID%>').val());
        $('#<%=zei_goukei.ClientID%>').val($('#<%=zei_goukeiran.ClientID%>').val());
        $('#<%=kin_goukei.ClientID%>').val($('#<%=kin_goukeiran.ClientID%>').val());
        var ojb = document.getElementById("hidden15");

        var div_element = document.getElementById('hidden15');
                   //動的に明細項目(hidden)サーバー転送用項目生成する
        var tr = $('#<%=GridView1.ClientID%> tr');
        for (var i = 1, l = tr.length ; i < l; i++) {
            var cells = tr.eq(i).children();//1行目から順にth、td問わず列を取得
            ...
            var kin = cells.eq(12).text();
                ...
                //部門
            var namewk = 'h_before_bumon_cd' + i;
            var wkelment_div = document.createElement("div_bumon_h" + i);
               wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
                //var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
                ...
                //部門
                var namewk = 'h_before_bumon_nm' + i;
                var wkelment_div = document.createElement("div_bumonnm_h" + i);
                wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
               // var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
                ...
                //科目
                var namewk = 'h_before_kamoku_cd' + i;
                var wkelment_div = document.createElement("div_kmokucd_h" + i);
                wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
                //var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
                ...
                //科目
                var namewk = 'h_before_kamoku_nm' + i;
                var wkelment_div = document.createElement("div_kmokunm_h" + i);
                wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
                //var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
                ...
                //内訳
                var namewk = 'h_before_uchiwake_cd' + i;
                var wkelment_div = document.createElement("div_uchiwakecd_h" + i);
                wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
                //var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
                ...
                //内訳
                var namewk = 'h_before_uchiwake_nm' + i;
                var wkelment_div = document.createElement("div_uchiwakenm_h" + i);
                wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
                //var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
                ...
                ////税区分名
                var namewk = 'h_before_zkubunnm' + i;
                var wkelment_div = document.createElement("div_zekubunnm_h" + i);
                wkelment_div.innerHTML = '<input type="hidden" name="' + namewk + '"' + 'id="' + namewk + '">';
                //var ojb = document.getElementById("hidden5");
                ojb.appendChild(wkelment_div);
    ...
    $('#<%=kounyu_date.ClientID%>').css('ime-mode', 'inactive');
    ...
    $('#d_keihi').click(function () {
        $('#d_keihi').prop('readonly', false);
        $('#d_keihi').css('background-color', 'white');
        $('#d_syozei').prop('readonly', false);
        $('#d_syozei').css('background-color', 'white');
    });

    $('#d_keihi').focus(function () {
        $(this).select();
    });


    $('#d_syozei').click(function () {
        $('#d_keihi').prop('readonly', false);
        $('#d_keihi').css('background-color', 'white');
        $('#d_syozei').prop('readonly', false);
        $('#d_syozei').css('background-color', 'white');
    ...
    $('#d_syozei').focus(function () {
        $(this).select();
    });


    $('#d_bumon_id').focus(function () {
        $(this).select();
    });

    $('#d_kan_code').focus(function () {
        $(this).select();
    });
    $('#d_uti_code').focus(function () {
        $(this).select();
    });
    $('#d_kingaku').focus(function () {
        $(this).select();
    });

  
    $('#d_tekiyo').focus(function () {
        $(this).select();
    });

    $('#<%=kounyu_date.ClientID%>').focus(function () {
        $(this).select();
    });
    ...
    $("input[type = text][id^='d_']").bind("keydown", function (e) {

        var n = $("input[type = text][id^='d_']").length;
        if (e.which == 13 || e.which == 9) {
            e.preventDefault();
        
            var nextIndex = $("input[type = text][id^='d_']").index(this) + 1;
            //index＝8は金額欄で最後
            if (nextIndex == 9) {
                $('#dialog_seikyu').siblings('.ui-dialog-buttonpane').find('button:eq(0)').focus();
            } else {
                if (nextIndex < n) {

                    if ($("input[type = text][id^='d_']")[nextIndex].disabled == true) {
                        $("input[type = text][id^='d_']")[nextIndex + 1].focus();
                    } else {
                        $("input[type = text][id^='d_']")[nextIndex].focus();
                    }
                } else {
                    //最後のやつなので#login-btnをクリック        
                    //  $('input[type = text]')[nextIndex - 1].blur();
    ...
    $("input[type = text][id='<%=kounyu_date.ClientID%>']").bind("keydown", function (e) {
        var n = $("input[type = text][id='<%=kounyu_date.ClientID%>']").length;
        if (e.which == 13 || e.which == 9) {
            e.preventDefault();
            $('#d_trade_cd').focus();
    ...
    for (var i = 1, l = tr.length ; i < l; i++) {
        var cells = tr.eq(i).children();//1行目から順にth、td問わず列を取得
       // cells.eq(8).addClass('grd1_notes');
        cells.eq(10).addClass('grd1_keihi');
        cells.eq(11).addClass('grd1_zei');
        cells.eq(12).addClass('grd1_kin');
   ...
   //*******
       ...
       //入力日の保持
    if ($('#<%=after_input_date.ClientID%>').val().length === 0) {

    } else {

        $('#<%=kounyu_date.ClientID%>').val($('#<%=after_input_date.ClientID%>').val());
    }

    var inputdate_wk = $('#<%=kounyu_date.ClientID%>').val();
    var jibumoncd = document.getElementById("auth_bumon_cd").value;
    var url_wk = 'Form_SEIKYUDN_New.aspx/GetLastCloseDate';
    var JSONdata = {
        code: jibumoncd,
    };
    ...
    $('#<%=kounyu_date.ClientID%>').prop('disabled', false);
    $('#trade_serach').prop('disabled', false);
    ...
    $.ajax({
        ...
        success: function (result) {
            var member = result.d;
            var closedate = member[0].CLOSEDATE;
            if (closedate >= inputdate_wk) {
          
                $('#<%=gyo_new.ClientID%>').prop('disabled', true);
                $('#<%=gyo_del.ClientID%>').prop('disabled', true);
                $('#<%=button_kousin.ClientID%>').prop('disabled', true);
                $('#<%=button_delete.ClientID%>').prop('disabled', true);
                $('#button_kakunin').prop('disabled', true);
                $('#d_trade_cd').prop('disabled', true); 
                $('#<%=kounyu_date.ClientID%>').prop('disabled', true);
                $('#trade_serach').prop('disabled', true);
                $('#<%=msgbox.ClientID%>').text('***** 締処理された伝票です *****');
                grid_click_flg = '1';
            }
            //20161228 sts ishida
            //締日日付の日のセット
            shimebi = closedate.substring(8, 10);
            return result;
        },
    ...
    $('#d_trade_cd').val($('#<%=h_trade_cd.ClientID%>').val());
    $('#l_trade_name').val($('#<%=h_trade_name.ClientID%>').val());
    $('#l_trade_address').val($('#<%=h_trade_address.ClientID%>').val());
    $('#d_cir_no').val($('#<%=h_cir_no.ClientID%>').val());
    ...
    diclose.dialog({
        ...
        buttons: {
            'はい': function () {
                $(this).dialog('close');
                $('#<%=btnClose.ClientID%>').trigger('click');
    ...
    di_denpyo_delete.dialog({
        ...
        buttons: {
            '削除': function () {
                ...
                url_wk = 'Form_SEIKYUDN_New.aspx/DeleteDenpyoJOSNData';
                $.ajax({
                    type: "POST",
                    url: url_wk,
                    data: JSON.stringify(JSONdata),
            ...
            //
            {
                text: titlename,
                  click: function () {
                      ...
                      var kin = document.getElementById("d_gokei").value;
                      ...
                      if (bumon_cd.length > 5) {
                          $('#msg').text('部門コード　桁数オーバーです');
                          err_flg = '1';
                      }

                      if (bumon_cd.length === 0) {
                          $('#msg').text('部門コードが未入力です');
                          err_flg = '1';
                      }
                      //200161208追加
                      if (bumon_cd.length > 0) {
                      ...
                      if (kamoku_cd.length > 0) {
                          ...
                          if (!kamoku_cd.match(/^(\w| |'|,|&)+$/)) {
                              $('#msg').text('科目コード半角以外の文字は使用できません');
                              err_flg = '1';
                          }
                      ...
                      var zei2 = zei.replace(/,/g, "");
                      if (zei2.length > 10) {
                          $('#msg').text('金額欄　文字数オーバーです');
                          err_flg = '1';
                      }

                      if (pattern.test(zei2)) {
                      } else {
                          $('#msg').text('金額が不適切です');
                          di_msg.dialog('open');
                          return false;
                      ...
                      if ($('input[name=z_kubun]:checked').val() === '2') {
                          zkubun = '3';
                          zname = '非課税';
                      }
                ...
                　　　　　//次の行に追加
                     ...
                     // $('#wk_tekiyo').val('');
                      ...
                      cells.eq(8).text(zkubun);
                      cells.eq(9).text(zname);
                      cells.eq(10).text(keihi);
                      cells.eq(11).text(zei);
                      cells.eq(12).text(kin);
           ...
           if ($('#<%=kounyu_date.ClientID%>').val().length > 10) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    var member = result.d;
                    var closedate = member[0].CLOSEDATE;
                    if (closedate >= $('#<%=kounyu_date.ClientID%>').val()) {
                        errchk_date = '1';
                        var msgstring = '既に締処理が実行されている日付です。　　';
                        msgstring = msgstring + '締処理取消を行ってから再入力してください。'
                        $('#msg').text(msgstring);
                        di_msg.dialog('open');
    ...
    $('#button_kakunin').click(function () {
        ...
        var torichk = $('#d_trade_cd').val();
        if (torichk.length > 0 ) {
            if (torichk.length > 6) {

                $('#msg').text('取引先コードが不正です');
                di_msg.dialog('open');
        ...
        if ($('#<%=syubetu.ClientID%>').val() === '新規') {
            ...
            //20161228 sts ishida
            //
            var date_koubay = new Date(koubay_date);
            var dddd = koubay_date_val.substring(6, 8);
            //
            if (dddd > shimebi) {
                ...
                var kounyu_date_YYYY = koubay_date_val.substring(0, 4);
                var kounyu_date_MM = koubay_date_val.substring(4, 6);
                if (kounyu_date_MM === '01') {

                    var kounyu_date_MM_before = '12';
                    var kounyu_date_YYYY_before = (parseInt(kounyu_date_YYYY) -1).toString();
                    kounyu_date_YYYY = kounyu_date_YYYY_before;


                }
                else {

                    var kounyu_date_MM_before = ('00' + (parseInt(kounyu_date_MM) -1).toString()).slice(-2);

                }
         
                YM = kounyu_date_YYYY.substring(2, 4) + kounyu_date_MM_before;
         
            }

            var inputym = YM;
            var url_wk = 'Form_SEIKYUDN_New.aspx/GetJOSNDenpyo_NO';
            var JSONdata = {
                //code: bumon_cd
                jibumoncd: jibumoncd,
                ym: inputym
            ...
            $.ajax({
                ...
                success: function (result) {
                    var member = result.d;
                    console.log(member);
                    var no = parseInt(member[0].MAXBILL_NO) + 1;
        ...
        $('#<%=after_input_date.ClientID %>').val($('#<%=kounyu_date.ClientID%>').val());

         if (errchk_kin == '9') {
            $('#<%=button_kousin.ClientID%>').prop('disabled', true);
      
            $('#msg').text('経費、税額、合計に不整合が存在します');
            di_msg.dialog('open');
    ...
    //閉じるボタンで変更があるのに更新せずクローズしたときのチェック
    ...
    var tr = $('#<%=GridView1.ClientID%> tr');
        for (var i = 1, l = tr.length ; i < l; i++) {
            ...
            //部門
            var namewk = 'h_before_bumon_nm' + i;
            if (document.getElementById(namewk).value === bumonnm) {
            } else {
                chk = '1';
            }
            ...
            //科目
            var namewk = 'h_before_kamoku_cd' + i;
            if (document.getElementById(namewk).value === kamokucd) {
            } else {
                chk = '1';
            }
            ...
            //内訳
            var namewk = 'h_before_uchiwake_cd' + i;
            if (document.getElementById(namewk).value === uchiwakecd) {
            } else {
                chk = '1';
            }
            ...
            ////摘要
            var namewk = 'h_before_tekiyo' + i;
            if (document.getElementById(namewk).value === tekiyo) {
            } else {
                chk = '1';
            }
            ...
            ////税区分名
            var namewk = 'h_before_zkubunnm' + i;
            if (document.getElementById(namewk).value === zname) {
            } else {
                chk = '1';
            ...
            ////経費
            var namewk = 'h_before_keihi' + i;
            if (document.getElementById(namewk).value === keihi) {
            } else {
                chk = '1';
            }
            ...
            ////消費税
            var namewk = 'h_before_zei' + i;
            if (document.getElementById(namewk).value === zei) {
            } else {
                chk = '1';
            }
            ...
            ////金額
            var namewk = 'h_before_kin' + i;
            if (document.getElementById(namewk).value === kin) {
            } else {
                chk = '1';
            }
    ...
    $('#d_trade_cd').on({
        change: function () {
            ...
            var trade_name = document.getElementById("l_trade_name").value ;
            var wk_TRADE_NAME = document.getElementById("l_trade_name");
            var ad = document.getElementById("l_trade_address");
            ad.value = '';
            ...
            if (trade_cd.length > 6 || trade_cd.length === 0) {
    ...
    $('#trade_serach').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.d.length > 0) {
                    //$('#trade').append("<tr style='background-color:#6699FF'><th>コード</th><th>取引先</th><th>住所</th></tr>");
                    $('#trade').append("<tr style='background-color:#6699FF'><th >コード</th><th>取引先</th><th>カナ</th><th>住所</th><th>口座番号</th><th>口座名</th></tr>");

                    for (var i = 0; i < data.d.length; i++) {
                        console.log (data.d[i].ACC_NUM);
                        //$('#trade').append("<tr onClick='mClickTR_trade(this)'><td>" + data.d[i].TRADE_CD + "</td> <td>" + data.d[i].TRADE_NM + "</td> <td>" + data.d[i].ADDRESS1 + "</td></tr>");
    ...
    $('input[name="z_kubun"]:radio').change(function () {
        calc_kingaku2($('#d_kingaku'), $('#<%=kounyu_date.ClientID%>').val());
...
　　//内税　
    ...
    $('#d_kan_code').on({
        change: function () {
            ...
            if (kamoku_cd.length > 0) {
                //科目名表示用関数
                //科目コードにより税区分の初期値をセット
                $('#d_kan_name').func_kamoku_nm_zei2(kamoku_cd, wk_KAMOKU_NAME);
    ...
    $('#kan_serach').click(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "Form_SEIKYUDN_New.aspx/GetJOSNData_Kamoku",
            contentType: "application/json;charset=utf-8",
            data: {},
            dataType: "json",
            success: function (data) {
    ...
    $('#uti_serach').click(function (e) {
        ...
        if (param_kamoku_cd.length === 0) {
            ...
            $.ajax({
                type: "POST",
                url: "Form_SEIKYUDN_New.aspx/GetJOSNData_Utiwake",
                contentType: "application/json;charset=utf-8",
                data: JSON.stringify(JSONdata),
                dataType: "json",
                success: function (data) {
    ...
    $('#d_kingaku').on({
        ...
        change: function () {
            ...
            //金額編集、消費税計算
            calc_kingaku2($(this), $('#<%=kounyu_date.ClientID%>').val());
    ...
    $("#MainContent_GridView1 td").dblclick(function () {
        ...
        var zkubun = $(this).closest("tr").find("td").eq(8).text();
        var zname = $(this).closest("tr").find("td").eq(9).text();
        var keihi = $(this).closest("tr").find("td").eq(10).text();
        var zei = $(this).closest("tr").find("td").eq(11).text();
        var gou = $(this).closest("tr").find("td").eq(12).text();
    ...
    $('#<%=gyo_del.ClientID%>').click(function () {
        ...
        if (cells.eq(1).text() > ' ') {
            ...
            var msg = document.getElementById("msg");
            msg.innerHTML = 'データのある行を選択してください';
    ...
    $('#d_keihi').on({
        change: function () {
            ...
            var keihi = document.getElementById("d_keihi").value;
            keihi = keihi.replace(/,/g, "");

            var zei = document.getElementById("d_syozei").value;
            zei = zei.replace(/,/g, "");
            var kei = parseInt(keihi) + parseInt(zei);
            var kei_elment = $('#d_gokei');
            var keihi_elment = $('#d_keihi');
            var syozei_elment = $('#d_syozei');

            $('#d_gokei').val(kei);
            kanma_func(kei_elment);
            kanma_func(keihi_elment);
            kanma_func(syozei_elment);
...
function mClickTR_trade(obj) {
        ...
        var nm = document.getElementById('l_trade_name');
        var ad = document.getElementById('l_trade_address');
...
function mClickTR_kamoku(obj) {
    ...
    if (kin_val > "") {
        ...
        if ($('input[name="z_kubun"]:eq(1)').is(':checked')) {
            ...
            $('#d_syozei').func_sotozeiritu(pdate, nm_zei, kin_val);
        }

        if ($('input[name="z_kubun"]:eq(2)').is(':checked')) {

            $('#d_keihi').val(kin_val_wk);
            $('#d_syozei').val('0');

        }

        if ($('input[name="z_kubun"]:eq(3)').is(':checked')) {
...
<asp:TextBox type="hidden" ID="h_trade_cd" runat="server"  ></asp:TextBox>
<asp:TextBox type="hidden" ID="h_trade_name" runat="server"  ></asp:TextBox>
<asp:TextBox type="hidden" ID="h_trade_address" runat="server"  ></asp:TextBox>
<asp:TextBox type="hidden" ID="h_cir_no" runat="server"  ></asp:TextBox>
 </div>
 ...
 <div class="hidden" id="hidden6">
      ...
      請求伝票入力 </div>
<div>
<asp:TextBox type="hidden" ID="after_input_date" runat="server"  ></asp:TextBox>
...
<div class="header0">
<asp:label runat="server" class="denban_lbl">請求番号</asp:label><asp:TextBox cssclass="denban input-sm" ID="denban1" name="denban1" runat="server" tabindex="-1" readonly="true"  ></asp:TextBox>
<asp:TextBox type="hidden" class="h_denban" ID="h_denban" name="h_denban" runat="server"  ></asp:TextBox>
   ...
   <table class="header_group1">
    <tr>
     <td>入力日<asp:TextBox cssclass="div_konyudate div_kounyu_date　datepicker input-sm" name="datei" id="kounyu_date" runat="server" style="margin-left:10px" ></asp:TextBox></td>
     <td></td> 
    </tr>
      <tr>
    <td>取引先<input type="text" name="d_trade_cd" id="d_trade_cd" class="input-sm" style="margin-left:10px"/><input id="trade_serach" type="button" class="btn btn-default" value="..." style="font-size :small">
    <input type="text" name="d_trade_name" id="l_trade_name"  class="input-sm" style="font-size:14px;border-style :none;margin-left:10px;background-color:whitesmoke;width:200px" readonly="readonly"/><input type="text" name="d_trade_address" id="l_trade_address"  class="input-sm" style="font-size:14px;border-style :none;background-color:whitesmoke;width:300px" readonly="readonly"/>
    </tr>
       <tr>
        <td>稟議No<input type="text" name="d_cir_no" id="d_cir_no" class="input-sm" style="margin-left:5px;width:300px"/></td>
      </tr>
</table>
        </asp:Panel>
    </div>
...
<asp:button cssclass="hidden btn btnClose btn-default" id="btnClose"  runat="server" style="font-size:larger" text="閉じる" />
<input type="button" class="btn btn-default" id="button_close" value="閉じる" style="font-size:larger"/>
...
<div id="dialog_trade" title="取引マスタ">
<div>
...
<!--新規登録画面の開始-->
<div id="dialog_seikyu" class="dialog_seikyu" title="請求伝票">
...
<div >
<input type="radio" id="d_z_kubun0" name="z_kubun" value="0" style="margin-top:10px;width:10px" /><label id="l_z_kubun0" style="margin-top:10px;width:100px;color:darkgreen ">内税</label>
<input type="radio" id="d_z_kubun1" name="z_kubun" value="1" style="margin-top:10px;width:10px" /><label id="l_z_kubun1" style="margin-top:10px;width:100px;color:darkgreen ">外税</label>
<input type="radio" id="d_z_kubun2" name="z_kubun" value="2" style="margin-top:10px;width:10px" /><label id="l_z_kubun2" style="margin-top:10px;width:100px;color:darkgreen">非課税</label>
<input type="radio" id="d_z_kubun3" name="z_kubun" value="3" style="margin-top:10px;width:10px" /><label id="l_z_kubun3" style="margin-top:10px;width:100px;color:darkgreen">その他</label>
</div>
<div>
<label>金額</label><input type="text" id="d_kingaku" class="input-sm" style="font-size:small;margin-left:5px;margin-top:10px;text-align:left  ; width:150px" />
</div>
...
<div id="dialogClose" title="メッセージ">
入力データが編集されています。
<br />
更新せず閉じますか?
<input type="text" class="hidden" id="close_flg" /> 
</div>
</div>
</asp:Content>
