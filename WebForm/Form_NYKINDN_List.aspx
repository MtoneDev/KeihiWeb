<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master" CodeBehind="Form_NYKINDN_List.aspx.vb" Inherits="KeihiWeb.Form_NYKINDN_List" %>
        ...
        change: function () {
           ...
           if ($('#<%=input_date1.ClientID%>').val().length > 10) {
            ...
            if (vMonth >= 0 && vMonth <= 11 && vDay >= 1 && vDay <= 31) {
             ...
             //$('#<%=input_date2.ClientID%>').datepicker('setDate', new Date(Date.parse(datefrom_chg)));
           $('#<%=input_date2.ClientID%>').focus();
        }
    });
...
//検索ボタン
    ...
    $('#<%=SearchClear.ClientID%>').click(function (e) {
        ...
        var url_wk = 'Form_NYKINDN_New.aspx/GetCloseKikan';
        var param_jibumoncd = document.getElementById("auth_bumon_cd").value;
        var JSONdata = {
        ...
        di_delete.dialog({
            ...
            buttons: {
                '削除': function () {
                    ...
                    //削除行から下の行を上にコピー                   
                    for (var i = gyo, l = tr.length - 2; i < l; i++) {
