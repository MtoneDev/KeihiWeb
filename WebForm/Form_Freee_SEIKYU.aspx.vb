...
Public Class Form_Freee_SEIKYU
    ...
    <WebMethod()>
    Public Shared Function CreateFreeeFile(ByVal vParam As String) As String
        Try
            ...
            'FREEE出力処理
            Dim lPath As String = CTL_GLOVIA.CreateSeikyuFile_Freee(lBumonList, lParam(0))
            Dim lFilename As String = Uty_File.GetFilename(lPath)
            lRstMsg.AppendLine("<br /><a href='../" & Uty_Config.OutputDir & "/" & lPath.Substring(lPath.LastIndexOf("\" & lBumonList(0) & "\") + 1) & "'>FREEEファイル請求書 (" & lFilename & ")</a>")

            'FREEE出力済みフラグ更新（CANCEL_FLG=2）
            'STS20220729
            Dim lCnt As Integer = CTL_GLOVIA.UpdateOutputGloviaFlg(lBumonList, lParam(0), 2)
            If lCnt <> lBumonList.Count - 1 Then
                lRstMsg.AppendLine("<p style='color:red;'>FREEE出力済みフラグの更新が正しくありませんでした。システム担当に連絡してください。</p>")
            End If
