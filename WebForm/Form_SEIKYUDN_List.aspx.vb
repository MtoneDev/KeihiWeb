...
Public Class Form_SEIKYUDN_List
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            tradecd = Request.QueryString("kensakucd")
            tradename = Request.QueryString("tradenm")
            If IsNothing(tradecd) Then
            ...
            Else
                ...
                h_tradecd.Text = tradecd
                h_tradename.Text = tradename
            End If
            ...
            'Ctrl.CTL_CT_CLOSE.GetCloseKikan(2, lLastShimebi, rstartdate, renddate)
            ...
            GridView_DispSelect(h_tradecd.Text, h_date_from.Text, h_date_to.Text)
            GridView_DispSelect2(h_tradecd.Text, h_date_from.Text, h_date_to.Text)
    ...
    Private Sub GridView_DispSelect(ByVal TRADECD As String, ByVal date1 As String, ByVal date2 As String)
        Try
            ...
            Dim lParam As New DS_BILL.SearchParamDataTable

            Dim row As DS_BILL.SearchParamRow
            'ヘッダー項目セット
            row = lParam.NewRow
            row.JIBUMON_CD = lJibumonCd
            row.TRADE_CD = TRADECD
            '日付け　文字に変換
            ...
            ldt = Ctrl.CTL_BILL.MeisaiSelectData(lParam)
    ...
    Private Sub GridView_DispSelect2(ByVal TRADECD As String, ByVal date1 As String, ByVal date2 As String)
        ...
        Try
            ...
            ldt = Ctrl.CTL_BILL.SelectData2(lParam)
            ...
            For i = 0 To GridView2.Rows.Count - 1
                ...
                GridView2.Rows(i).Cells(5).Text = GridView2.Rows(i).Cells(5).Text.Substring(0, 10)
    ...
    Private Sub GridView1_RowCreated(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowCreated
        ...
        Try
            ...
            If row.RowType = DataControlRowType.Header Then
                ...
                row.Cells(1).Text = "請求番号"

                row.Cells(3).Text = "取引先名"

                row.Cells(4).Text = "入力日"
                row.Cells(5).Text = "稟議No"
    ...
    Private Sub GridView2_RowCreated(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView2.RowCreated
        ...
        Try
            ...
            If row.RowType = DataControlRowType.Header Then
                ...
                row.Cells(1).Text = "請求番号"
                row.Cells(2).Text = "取引先CD"
                row.Cells(3).Text = "取引先名"
                row.Cells(4).Text = "稟議No"
                row.Cells(5).Text = "入力日"
                row.Cells(6).Text = "経費"
    ...
    Protected Sub SearchButton_Click(sender As Object, e As EventArgs) Handles SearchButton.Click
        Try
            ...
            Dim tradecd As String = Request.Form("d_trade_cd")
            Dim tradename As String = Request.Form("d_trade_name")
            Dim tradeaddress As String = Request.Form("d_trade_address")

            GridView_DispSelect(tradecd, input_date1.Text, input_date2.Text)
            GridView_DispSelect2(tradecd, input_date1.Text, input_date2.Text)
            ...
            h_tradecd.Text = tradecd
            h_tradename.Text = tradename
            h_tradeaddress .Text = tradeaddress
    ...
    Protected Sub Button_Disp_Click(sender As Object, e As EventArgs) Handles Button_Disp.Click
        Try
            ...
            Dim lrow As DS_BILL.SearchParam_ListRow
            lrow = lparam.NewSearchParam_ListRow()


            h_tradecd.Text = Request.Form("d_trade_cd")
            h_tradename.Text = Request.Form("d_trade_name")
            ...
            If Not IsNothing(gyowk) And gyowk <> "" Then
                ...
                Else
                    ...
                    input_date = GridView2.Rows(gyo).Cells(5).Text
                ...
                Dim ldt_trade As New DS_M_TRADE.M_TRADEDataTable

                Dim wkcode_trade As String = ldt(0).TRADE_CD
                Dim wkaddr As String = ""
                ldt_trade = CTL_M_TRADE.SelectData(wkcode_trade)
                If ldt_trade.Count > 0 Then

                    wkaddr = ldt_trade(0).ADDRESS1

                End If


                Dim url As String = ""
                url = url + "Form_SEIKYUDN_New.aspx?manageno=" & manage_no + "&jibumoncd=" + jibumon_cd + "&slipno=" + slip_no + "&inputdate=" + input_date + "&disp=" + dispwk + "&tradecd=" + ldt(0).TRADE_CD
                url = url + "&tradename=" + ldt(0).TRADE_NM + "&input_date1=" + input_date1.Text + "&input_date2=" + input_date2.Text
                url = url + "&cir_no=" + ldt(0).CIR_NO + "&tradeaddress=" + wkaddr + "&kensakucd=" + h_tradecd.Text
